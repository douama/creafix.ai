...
/**
 * POST /api/checkout/paydunya
 * Body : { amount, description, planId?, customerPhone? }
 * Currency : XOF (PayDunya = UEMOA only)
 */
export async function POST(request: Request) {
  ...
  // ─── 1. Insère la ligne payment PENDING (UUID local = référence stable) ───
  ...
  // ─── 2. Crée la facture chez PayDunya ───
  const invoice = await createInvoice({
    ...
    cancelUrl: `${APP_URL}/dashboard/billing?status=cancelled&pid=${payment.id}&provider=paydunya`,
  ...
  // ─── 3. Persiste le token PayDunya comme external_id ───
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  await (sb.from("payments") as any)
    .update({ external_id: invoice.token })
    .eq("id", payment.id);

  return NextResponse.json({ url: invoice.invoiceUrl, paymentId: payment.id });
}
